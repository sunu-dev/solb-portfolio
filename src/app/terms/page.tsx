  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', padding: '48px 24px', fontFamily: "'Pretendard Variable', sans-serif", background: 'var(--bg, #fff)', minHeight: '100vh' }}>
      ...
      <Section title="제2조 (서비스의 정의)">
        서비스는 AI 기반 주식 포트폴리오 분석 정보를 제공하는 웹 애플리케이션(PWA)입니다. 서비스는 증권의 매매·중개 기능을 제공하지 않으며, 이용자가 직접 입력한 보유 종목의 관리·확인 및 정보 제공을 중심으로 운영됩니다. 서비스는 자본시장법상 투자자문업, 투자일임업, 유사투자자문업 어디에도 해당하지 않으며, 불특정 다수에게 정보를 제공할 목적으로만 운영됩니다. 서비스는 어떠한 형태로도 종목 조언이나 매매 권유의 대가를 받지 않습니다.
        <br /><br />
      ...
      <Section title="제4조 (회원 가입 및 계정)">
        <ol>
          ...
          <li>서비스는 만 18세 이상만 가입하고 이용할 수 있습니다. 생년월일은 브라우저에서 성인 여부를 확인하는 데만 사용하며, 서비스는 생년월일 자체를 저장하거나 외부로 전송하지 않습니다.</li>
      ...
      <Section title="제5조 (서비스의 내용)">
        서비스는 다음 기능을 제공합니다.
        <ol>
          <li>주식 포트폴리오 관리 (종목 직접 추가, 수정, 삭제)</li>
          <li>실시간 시세 정보 (약 15분 지연)</li>
          <li>AI 기반 기술적 분석 및 멘토 6인 관점 분석</li>
          <li>시장 관찰판 — 모든 사용자에게 동일한 공개 기준 기반 정보 목록</li>
        ...
        <p style={{ marginTop: 12, fontSize: 13, color: 'var(--text-tertiary, #8B95A1)' }}>
          스크린샷 가져오기는 개인정보 보호 기준을 충족한 AI 처리 환경이 준비된 이후 별도 제공될 수 있으며, 현재는 비활성화되어 있습니다.
        </p>
        <p style={{ marginTop: 12, fontSize: 13, color: 'var(--text-tertiary, #8B95A1)' }}>
          <strong>고위험 상품 취급 범위</strong>: 단일종목 레버리지·인버스 ETF·ETN(예: 미래에셋 레버리지 삼성전자 단일종목 ETN, KODEX/TIGER/ACE 단일종목 2X·-2X ETF, 미국 TSLL/NVDU 등)은 일일 N배 추종·음의 복리·발행사 신용 위험이 있는 고위험 단기 트레이딩 도구입니다. 서비스는 해당 상품을 시장 관찰판·universe 편입 등 신규 발굴 대상에서 영구 제외하며, 종목 점수·AI 종목 분석·멘토 분석 대상에서도 제외합니다(분석 요청 시 고위험·적합성 의무 상품임을 안내하고 분석을 제공하지 않습니다). 이용자가 직접 검색·보유 등록하여 시세·보유 현황·수익률 등을 관리하는 것은 가능하나(성인 확인과 위험 고지 동의 선행), 매수·매도 방향이나 투자 판단은 일절 제공하지 않습니다.
        </p>
      </Section>
        ...
        <ol>
          <li>AI 기능은 다음과 같이 제한됩니다 (베타 기간 중 변경될 수 있음):
            <ul style={{ marginTop: 6, marginLeft: 12 }}>
              <li><strong>시장 관찰판</strong>: 로그인 사용자에게 동일하게 일 1회 제공됩니다.</li>
              <li><strong>AI 분석(기술적·멘토)</strong>: 로그인 필수. 멤버십과 관계없이 동일하게 일 3회 제공됩니다.</li>
      ...
      <Section title="제7조 (투자 관련 면책)">
        <ol>
          <li>서비스에서 제공하는 모든 분석, 점수, 알림, 시장 관찰판, 멘토 설명, 통합 평단가 계산값 등은 AI 또는 자동 알고리즘이 정리한 참고 자료이며, 투자 자문이나 종목 추천이 아닙니다.</li>
          <li>시장 관찰판을 포함한 모든 종목 정보는 특정 금융투자상품의 매수 또는 매도를 권유하지 않으며, 그 결과에 대한 어떠한 보장도 하지 않습니다.</li>
          ...
          <li><strong>단일종목 레버리지·인버스 상품(ETF·ETN) 관련</strong>: 본 상품은 일일 수익률의 N배(예: 2배 또는 -2배)를 추종하는 구조로, 장기 보유 시 음의 복리(volatility decay)로 인해 기초자산이 횡보하거나 변동성이 클 경우 원금이 잠식될 수 있습니다. ETN의 경우 발행사 신용 위험(채무불이행 시 원금 손실)이 추가로 존재합니다. 거래소(KRX) 및 금융감독원은 사전 교육 이수와 기본 예탁금 요건을 두고 있으며, 손실 감내 능력과 위험 이해도가 낮은 투자자에게는 부적합하다고 명시하고 있습니다. 서비스는 해당 상품을 신규로 추천하거나 관찰 후보로 제시하지 않으며, 종목 점수·AI 분석·멘토 분석도 제공하지 않습니다. 이용자가 직접 보유 등록하여 시세·보유 현황을 관리하는 것은 가능하나(성인 확인·위험 고지 동의 선행), 매수·매도 권유나 투자 판단은 일절 제공하지 않습니다.</li>
      ...
      <Section title="제15조 (베타 서비스 특수 조항)">
        <ol>
          ...
          <li>정식 서비스 전환 시 장기 기록 보관·이전 버전 복구·고급 예약 내보내기 등 비투자 관리 자동화 일부가 유료(PRO)로 제공될 수 있습니다. 기본 CSV·JSON 수동 내보내기는 무료로 유지하며, 투자 관련 AI의 횟수·품질·속도는 멤버십에 따라 차등하지 않습니다.</li>
