...
export default function TaxEstimateModal({ onClose }: Props) {
  ...
  const yearOptions = [thisYear, thisYear - 1]; // 올해 실현분 / 작년 귀속(올해 5월 신고)
    ...
    >
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--surface, #fff)', borderRadius: 20, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 24px 24px' }}>
            ...
            {yearOptions.map((y) => (
              <button
                ...
                style={{
                  ...
                  border: taxYear === y ? '1px solid var(--brand-primary, #0E7C7B)' : '1px solid #E5E8EB',
              ...
              <div key={row.id} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <div style={{ flex: 1, minWidth: 0, position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <input
                    ...
                    placeholder="실현손익"
                    aria-label="실현손익(만원)"
                    style={{ width: '100%', padding: '10px 40px 10px 12px', fontSize: 13, fontWeight: 600, border: '1px solid #E5E8EB', borderRadius: 10, outline: 'none', boxSizing: 'border-box', textAlign: 'right', color: row.gainKrw >= 0 ? GAIN : LOSS }}
                  />
          ...
          {/* 결과 */}
          {entries.length > 0 && (
            <div style={{ marginTop: 14, padding: 18, background: '#F8F9FA', borderRadius: 16 }}>
              {brokerCount >= 2 && (
