...
//
// ⚠️ v1 합산기(computeTaxEstimate): 증권사가 제공한 '실현손익(KRW)'을 **합산**해 예상 양도세를 추정한다.
//    세액을 '재계산'하지 않음(환율·lot·환차는 증권사가 이미 반영) — verified 공제·세율만 적용(세무사 감수 불요 경로).
//    결과는 '추정치'(신고 근거 아님). 화면에서 항상 '(추정)' 노출.
// ⚠️ v2 자체계산(toKrwAtSettle 등): raw 거래내역에서 직접 산출 — 세무사 감수+E&O 게이트(미구현).
//    상세: docs/legal-review/LEGAL_CONSULTATION_TAX.md A섹션.
