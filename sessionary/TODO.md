...
> **최신 sessionary**: `2026-08-20-macro-indicators-and-edu-tips.md`
>
> **완료**: PR 4건 머지(#32~#35). 미 10년물 1개 → **5지표**(10년물·미 기준금리·CPI·실업률·일본 단기금리)
> + **발표 캘린더**(FOMC·CPI·고용·BOJ 금정위, 공식 소스 직접 확인) + **교육 팁 심화**.
> 전부 무료·키 불요 공공 원천(재무부·FRED·BLS·BOJ), cron·AI·유료 API 0.
>
> **§6 판정 근거 교정**: '3요건'은 내부 위험 모델이고, **자본시장법 제7조 제3항**이
> "간행물·통신물 등을 통한 **개별성 없는 조언**"을 투자자문업에서 **명시 제외**한다(원문 확인).
> → 모두에게 동일한 정적 일방향 팁은 더 깊게 설명 가능. 카드 펼침을 3문장 → **6구조**로
> (정의→계산 원리→함께 볼 것→해석 범위→숫자 읽기). **깊이의 한계 = "방향 대신 계산 원리"**.
> 푸시는 법이 아니라 `Status≠Alert` 제품 정책이 막는다(정책 SSOT 변경 선행 필요).
>
> **후속(같은 날)**: 일본 재감사 재실행(15/15 완주) → 실재 5건 수정(PR #37) ·
> **BOJ 공식 API v1 전환**(168KB 화면 CSV → 8KB 문서화 경로) + 의무 크레딧 표기(PR #39).
> 재감사가 잡은 것 중 중요한 둘: ①일본 카드가 SSOT에 '한국 증시'를 처음 쓰면서
> **그 대상어가 §6 가드에 없어 인과문 13/14가 통과**하던 것(보강 후 탈출 0/10)
> ②`changePp` 3자리 계산 → 2자리 표시로 관측치 41%가 **`-0.00%p`** 자기모순 표기.
>
> **검증**: verify exit 0 · 테스트 **793**(08-19 시작 476) · 미커밋 0 · 프로덕션 배포 확인(5지표).
>
> **🔴 파운더 남은 2건**: ①**Finnhub API 키 회전**(유출 간주 지속)
> ②**BOJ 조사통계국 통보**(api_notice §1 의무, `post.rsd17@boj.or.jp`).
> 육안 검증은 완료, 한국·일본 CPI 확장은 키 발급까지 **보류**.
> ---

## 🗄️ (직전) 브리핑 (2026-08-19 — 수급·매크로 이중 분석 → 미 10년물 v1 출시)
...
### 🆕 2026-08-20 — 매크로 지표 후속
- [x] ~~**2단계 일본 적대 재감사 재실행**~~ — 재실행 15/15 완주, 발견 22건. 실재 확정분 5건 수정 후 PR #37 머지 (2026-08-20)
  - **§6 가드 사각(높음)**: 일본 카드가 SSOT에 '한국 증시'를 처음 등장시켰는데 대상어가 가드에 없어
    인과문 13/14가 통과했다(내 프로브 5/6 탈출 재현). 상품어·방향어·조건절 보강 → 탈출 0/10·과차단 0/5
  - **자기모순 표기(높음)**: changePp 3자리 계산 → 2자리 표시로 관측치 41%가 `-0.00%p`. 2자리로 통일
  - BOJ 시각 미고지인데 `12:00` 단정 → `timeKnown` 플래그 / BOJ 장애가 미국 캐시 15배 깎던 것 / aria 수치 누락
- [x] ~~**BOJ 공식 API v1 전환**~~ — 완료(PR #39). 168KB 화면 CSV → 8KB 공식 API.
  따옴표 안 쉼표 때문에 파서를 끝에서부터 읽도록 신설, 결측 `null`·STATUS 비200 처리, 폴백 유지.
  **의무 크레딧은 카드 하단 출처에 지정 원문 표기**(파운더 결정) (2026-08-20)
- [ ] 🔴 **BOJ에 서비스 공개 통보 (파운더 액션)** — api_notice.pdf §1 의무.
  메일 `post.rsd17@boj.or.jp` / 제목 `【検索サイトAPIを利用したサービス公開】`.
  API를 실제로 쓰는 서비스가 배포됐으므로 지금 발송 대상이다.
- [ ] **BOJ 데이터 상업 이용 — 법률 게이트 등재** — 상업 서비스에서의 전재·복제는 BOJ 사전 상담 대상.
  현재는 무료라 미해당이나 **유료화(P1) 시점에 발동**한다. `PROFESSIONAL_REVIEW_REGISTER.md`에
  데이터 소스 라이선스 항목이 0건이라 등재 필요
- [x] ~~**5지표 카드 육안 검증**~~ — 파운더 확인 완료 (2026-08-20)
- [~] **3단계 한국 — 보류(파운더 결정 2026-08-20)**. ECOS 무료 인증키 발급이 선행 조건이라 대기.
  재개 시 참고: OECD 물가 데이터(`DSD_PRICES@DF_PRICES_ALL`)에는 **한국이 존재**하므로
  CPI만이라면 키 없이도 가능하다(일본은 부재였음, 2026-08-20 실측). 기준금리는 ECOS 필요.
- [~] **일본 CPI — 보류(한국과 같은 사유)**. FRED 계열 2021년 중단·OECD에 일본 부재라
  e-Stat(총무성) 무료 키가 유일 경로. 키 발급 시 재개.
- [ ] **매크로 푸시 알림 판정** — 법적으로는 사실 통지(일정·결과)까지 여지 있으나 `Status≠Alert`
  정책이 막는다. 열려면 ①정책 SSOT에 '옵트인 매크로 통지' 예외 신설 ②BLS 6h 캐시로는 '발표 즉시'가
  불가하므로 수집 배선 변경 ③fail-closed 송신 게이트(`validateAlertMessage`는 prod에서 **차단하지 않음**)
...
> **🔴 다음(차트)**: ① 운영 육안 검증(해설 톤·문구 다듬기·칩 중립색·$/₩ 정렬·다크) — 파운더 피드백 대기 ② (선택) chart_guide_expand 펼침률 보고 기본 펼침 A/B.
> **⚠️ §6 사각 룰**: 정적 분석/차트/요약 카피는 lint:alerts(경로한정)·alertCompliance(알림전용) 안 본다 → 새 해설 기능엔 누출 불변식 테스트 필수. 초보 설명: 서술까지만·앞일 양쪽+"아무도 모름"+참고용·색 valence(매수/매도) 금지.
> **🔴 상위 painkiller(미진행 유지)**: **세무 v1 카나리(BLOCKER#1·유일 painkiller)** — 14인 패널 1순위, 프리뷰 READY·파운더 액션·세무사법 §20③ 감수 게이트.
> ---

## 🗄️ (직전) 브리핑 (2026-06-21 — 투어 강화, 완결)
...
> **🟢 tour_events 마이그 적용 완료(파운더) + Phase 4 진행형 체크리스트 완료(PR #21)**: GettingStartedChecklist 5항목 자동체크(feature_first_use), 신규자 14일 게이트·분석/멘토 첫보유 패널·a11y. **투어 강화 Phase 0~4 전부 완결(PR #12~#21, 10 PR).**
> **🔴 투어 잔여(소·선택)**: ① demo_to_login funnel from='demo-banner' 분기(분석 정밀도) ② checklist_* 텔레메트리를 TOUR_EVENT_NAMES 화이트리스트로 통일(SSOT 일관성). 둘 다 비차단.
> **🔴 상위 painkiller(미진행 유지)**: **세무 v1 카나리(BLOCKER#1·유일 painkiller)** — 14인 패널 1순위, 프리뷰 READY·파운더 액션·세무사법 §20③ 감수 게이트.
...
## 🗄️ (구) 브리핑 (2026-06-21 darkcore — 8 PR 종결)
