...
- [ ] **USD 자릿수 관례 3종을 하나로 통일할지 결정** — 현재 `formatUsd(v)`(2자리 고정) / `{max:2}` / `{max:3}` / `{max:0}`가 공존. 통합하면 `$480` ↔ `$480.00` 같은 표기가 바뀌므로 제품 판단 필요
...
- [ ] **`formatRelativeKo` 도입** — 뉴스 갱신 배지 등 "방금 · N분 전" 표기. 현재 앱에 상대 시간 표기가 아예 없다
