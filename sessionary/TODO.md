...
- [ ] **BOJ 공식 API v1 전환** — 현행 수집 경로는 BOJ가 '예고 없이 변경·삭제 가능'이라 **명시한**
  화면 다운로드 URL(168KB 전량). 공식 API v1은 문서화돼 있고 1KB·`text/csv; charset=utf-8`(실측 확인).
  ⚠️ 전환 시 **의무 2건**: ①지정 크레딧 문구를 이용자가 볼 수 있는 곳에 표기
  ②조사통계국(post.rsd17@boj.or.jp) 통보. 파서도 새로 필요(메타 12줄 + `SURVEY_DATES` 기반, 결측은 `null`).
  → **파운더 판단 필요**(크레딧 문구 노출 위치)
