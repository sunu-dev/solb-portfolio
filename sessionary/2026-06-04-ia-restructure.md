...
> **결말**: 사용자 프리뷰 확인 + 데스크톱 모달 폴리시(상단여백·닫기X) 후 "지금 기준으로 푸쉬하자" → **ia-p0 → main 머지·production 배포 완료(`e919dcb`)**. IA 재정비 종결. 후속 없음.
...
- [x] ~~`ia-p0` 프리뷰 검토 → main 머지~~ **완료**: 머지·production 배포(`e919dcb`). 5청크 + dead code 삭제로 번들 감소.
- [ ] (정리, 선택) 머지 완료된 원격 `ia-p0` 브랜치 삭제 — 단, 삭제 시 프리뷰 별칭 404. 당장은 유지.
- [ ] (설계 수용분, 일부 해소) 모바일 관심종목 진입 — '전체' 허브에 관심종목 바로가기 추가됨(currentTab). 추가 딥링크 필요 시만.

## 다음 세션 진입점

IA 재정비는 **production 배포 완료(`e919dcb`)**로 종결. 다음 신규 작업의 최우선은 **세무 v1 슬라이스 카나리 WTP 검증(BLOCKER #1, `tax-v1-slice` 브랜치)**. IA 관련 후속 작업은 없음(원하면 운영 화면 모니터링/추가 폴리시만).
