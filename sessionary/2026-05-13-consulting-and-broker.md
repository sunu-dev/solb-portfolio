...
### E-1. Today30sCard 제거 (회고)

P0-5 작업으로 추가했던 `Today30sCard.tsx` — 사용자 피드백 "계속 남아있고 영역 크다"로 제거 결정.

**문제**:
- 의도(모닝브리프+챕터+CTA 통합)와 달리 실제로는 시간 인사 + 보유 종목 카운트 + CTA 2개에 그침
- MorningBriefing 컴포넌트가 이미 모닝브리프 역할 → **중복 카드**
- 챕터 진척은 MonthlyChapter가 담당 → 중복
- 닫기 없음 → 매일 노이즈

**교훈**: "통합 카드" 설계 시 기존 컴포넌트와의 역할 분담 사전 검토 필요. 추가 전에 정말 통합 가치를 주는지 페르소나로 시뮬해야.
